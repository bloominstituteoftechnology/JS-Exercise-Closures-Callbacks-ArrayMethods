...
function processLastItem(stringList, callback) {
  /* CODE HERE */
  let itemLast = stringList.pop();
  return callback(itemLast)
}
...
function processSum(numberList, callback) {
  let sum = numberList.reduce((nums, value) => {return nums + value;}, 0);
  return callback(sum)
  /* CODE HERE */
}
...
function tallyUpDonations(runners) {
  let sum = (cue) => {
    return runners.reduce((a, b) => a + (b[cue] || 0), 0);}
    return sum('donation');
}
 ...
 * etc
*/
function counterMakerWithLimit(numMax) {
  /* CODE HERE */
  let count = 0
