...
function getFullNames(runners) {
  ...
  runners.forEach(element => {
    fullName[i] = element.last_name + ', ' + element.first_name;
    i++
  });
...
function firstNamesAllCaps(runners) {
  ...
  const capitalFirstName = runners.map(runners => runners.first_name.toUpperCase());
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  /* CODE HERE */
  const runnersBySize = runners.filter(runners => runners.shirt_size === tShirtSize);
...
function tallyUpDonations(runners) {
  /* CODE HERE */
   ...
   const totalDonations = runners.reduce((total, runners) => {
     return total += runners.donation}, 0);
