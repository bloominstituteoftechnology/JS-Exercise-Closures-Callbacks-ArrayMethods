...
function processSum(numberList, callback) {
  const accumulator = (accumulator, currentValue) => accumulator += currentValue
    return callback(numberList.reduce((accumulator), 0));
  }
/**
...
function processDuplicateFree(list, callback) {
  const unique = (value, index, self) => {
    ...
    return callback(list.filter(unique))
}
...
function tallyUpDonations(runners) {
  return runners.reduce((total, person) => total + person.donation, 0);
}
