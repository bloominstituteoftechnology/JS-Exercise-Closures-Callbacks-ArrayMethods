...
function processSum(numberList, callback) {
  return callback (numberList.reduce((add1,add2)=>add1+add2,0));
  }  //help 
 ...
 * should return 1000.
*/
function processProduct(nums1,nums2,callback) {
  callback(nums1*numbs2);
...
*/
function getFullNames(runner) {
  return runner.map(arr=> `${arr.last_name}, ${arr.first_name}`);
...
function firstNamesAllCaps(runners) {
  return runners.map(arr=>arr.first_name.toUpperCase());
}
...
function getRunnersByTShirtSize(runners,tShirtSize) {
  return runners.filter(arr=>arr.shirt_size==tShirtSize);
}
