...
function processSum(numberList, callback) {
  var sum = numberList.reduce(function(sum, currentval) {
...
function processContains(item, list, callback) {
  ...
  // init a variable called bool. filter the array, if an aray item is === to the item argument will exist 
  if(bool.length > 0){
    // if the variable exists, length is greater than 1 and will return callback == true
    return callback(true);
  }
    else return callback(false); 
    // if the variable does not exist, because item !== list, so return false
}
...
function processDuplicateFree(list, callback) {
  let arg = list.filter((item, index) => list.indexOf(item) === index);
  return callback(arg)
}
...
let fullName = [];
runners.forEach (function(objects){ 
fullName.push (`${objects.last_name}, ${objects.first_name}`)
});
return fullName; 
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const correctSize = runners.filter(function(item){
...
function tallyUpDonations(runners) {
  const tally = runners.reduce (function(totalsofar, current){
    return totalsofar + current.donation; 
  }, 0);
  return tally; 
}
...
function counterMaker() {
  // BROKEN CODE STARTS
  let count = -1; //for first iteration to start at 0 then variable should init @-1. changed const to let as const is protected. 
  return function counter() { //needed to add return function
    return ++count;
  }
  // BROKEN CODE ENDS
}
...
/*{
  let count = -1; // init count at -1 so first iteration = 0
  function counter (){ 
    return ++count; 
  }
  if (count >= num) {let count = -1;} // resets count if count > or = num argument
  return counter;  
} */
