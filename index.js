...
// 👇 COMPLETE YOUR WORK BELOW 👇
// 👇 COMPLETE YOUR WORK BELOW 👇
processFirstItem([], dkindex1 => {
  console.log(dkindex1);
});
...
function processLastItem(/* CODE HERE */ stringList, callback) {
  ...
  // return callback(stringList.find((item,index) => {index === (stringList.length)- 1;}));
}
processLastItem([], str1 => {
  return str1 + str1;
});
 ...
 */
function firstNamesAllCaps(/* CODE HERE */runers) {
  /* CODE HERE */
  let full_nam = [];

  runers.forEach(runers =>
    full_nam.push(`${runers.last_name}, ${runers.first_name.toUpperCase()}`)
  );
  return full_nam;
}
/**
...
function getRunnersByTShirtSize(/* CODE HERE */runners, tShirtSize) {
  /* CODE HERE */
  return runners.filter(runner=>runner.shirt_size===size);
}
