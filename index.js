...
function isItAnApple(strings) {
  let appleArray = strings.map(item => {
    if (item === "apple") {
      return true;
    } else {
      return false;
...
function removeApple(strings) {
  let noApple = strings.filter(item => item !== "apple");
  return noApple;
}
...
function stringSmash(strings) {
  let arrayStrings = strings.reduce((acc, currValue) => acc + currValue);
  return arrayStrings;
}
...
function getFullNames(runners) {
  const runNames = runners.map(runner => {
...
function firstNamesAllCaps(runners) {
  let teamNames = runners.map(runnerName =>
    runnerName.first_name.toUpperCase()
  );
  return teamNames;
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  let runnersWithShirtSize = runners.filter(
...
function tallyUpDonations(runners) {
  let finalTotal = runners.reduce(function(total, donation) {
