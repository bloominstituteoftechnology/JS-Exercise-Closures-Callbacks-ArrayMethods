...
function processContains(item, list, callback) {
  ...
  return callback(filterItem);/* CODE HERE */
}
...
function firstNamesAllCaps(runners) {
  const arr = [];
  runners.map(person => arr.push(`${person.first_name.toUpperCase()}`)); 
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  return runners.filter( person => person.shirt_size === tShirtSize);/* CODE HERE */
}
...
function tallyUpDonations(runners) {
  return runners.reduce((total, person) => total + person.donation, 0);/* CODE HERE */
}
