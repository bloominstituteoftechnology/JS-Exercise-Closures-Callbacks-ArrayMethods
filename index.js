...
function processContains(item, list, cb) {
  let containsItem = list.includes(item);
...
function getFullNames(runners) {
  ...
  runners.forEach(function(items){
    namesArray.push(`${items.last_name}, ${items.first_name}`);
  });
  return namesArray;
}
...
function firstNamesAllCaps(runners) {
 let namesArray = runners.map(function(runner){
...
function getRunnersByTShirtSize(runners, shirtSize) {
  let runnersBySize = runners.filter(runner => runner.shirt_size === shirtSize);
...
function tallyUpDonations(runners) {
  let sum = runners.reduce(function(acc, item){
