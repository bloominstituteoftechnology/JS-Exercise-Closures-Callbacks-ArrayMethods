...
function processSum(numberList, callback) {
  let sum = numberList.reduce((hold, num) => hold += num, 0);
  return callback(sum);
  }
...
function getFullNames(runners) {
  ...
  runners.forEach(function(array){
    return hold.push(`${array.last_name}, ${array.first_name}`);
  });
  return hold
}
...
function tallyUpDonations(runners) {
  return runners.reduce((here,there)=> { 
    return here += there.donation
  },0)
}
