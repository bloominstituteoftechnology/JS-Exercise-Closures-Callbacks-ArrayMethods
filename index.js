...
function processSum(numberList, callback) {
  var List = numberList.reduce((accumulator, next) =>{
...
function getFullNames(runners) {
  const runnersName =[];
  runners.forEach(runners => {runnersName.push(`${runners.last_name}, ${runners.first_name}`)})
  return runnersName;
}
...
function firstNamesAllCaps(runners) {
  const capitalNames = runners.map(runners => runners.toUpperCase);
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const shirt =[]
  let item = runners.filter(runners => {
    for (let i =0; i < runners.length; i++)
...
function tallyUpDonations(runners) {
  const item = runners.length;
  let donations = runners.reduce((acc, currentValue) => {
    return acc += currentValue.donations;
  }, 0)
}
...
function counterMaker() {
  // BROKEN CODE STARTS
  const count = 0
  const counter = function() {count - count +1; return count};
    return counter;
  }
  // BROKEN CODE ENDS
