let runners = require('./data/runners');
...
const array = ['Volleyball', 'Rock Climbing', 'Hiking', 'Fishing'];
function processLength(array, cb) {
  return cb(array.length);
}

console.log(
  processLength(array, (arrayLength) => {
    return arrayLength
  })
)
...
console.log(
  processLastItem(array, (lastItem) => lastItem)
)
...
function processSum(numberList, cb) {
  return cb(numberList.reduce((acc, cur) => acc += cur, 0))
}

console.log(
  processSum(numbers, (sum) => sum)
)
...
console.log(
  processProduct(2, 7, (product) => product)
)
...
function callback(foundItem) {
  return foundItem
}

console.log(
  processContains(6, numbers, callback)
)

console.log(
  processContains(1, numbers, callback)
)
...
const dupeNumbers = [1,1,2,2,3, 4, 4];
function processDuplicateFree(array, cb) {
  // let nonDupeNumbers = [];

  // function includes(num, array) {
  ...
  // for (var i = 0; i < array.length; i++) {
  //   if (!includes(array[i], nonDupeNumbers)) {
  //     nonDupeNumbers.push(array[i])
  //   }
  // }

  // return cb(nonDupeNumbers)
  // return cb(array.filter((num, index) => array.indexOf(num) === index))
  return cb(
    array.reduce((nondupes, num) => nondupes.includes(num) ? nondupes : nondupes.concat(num), [] )
    )
}

console.log(
  processDuplicateFree(dupeNumbers, (nonDupeNumbers) => nonDupeNumbers)
)
...
function getFullNames(list) {
  list.forEach((runner) => runner.full_name = `${runner.last_name}, ${runner.first_name}`);
  return list.map((runner) => runner.full_name)
  // return list.map((runner) => `${runner.last_name}, ${runner.first_name}`)
}
...
function counterMakerWithLimit(num) {
  let counter = 0;
  const max = num;
  return function counterWithLimit() {
    if (counter > max) {
      counter = 0
    }
    return counter++;
...
const x = counterMakerWithLimit(3);
x()
x()
x()
console.log(x())
